...
class CopyAttentionDistribution(nn.Module):
    ...
    def forward(self,current_hidden_state,bert_inputs,source,output_size,CopyCoverageVector):
        ...
        source_position_copy_prob = torch.zeros((batch_size,output_size),dtype=torch.float32).cuda()
        ...
        for i in range(batch_size):
            ...
            mask = torch.zeros((max_time_step,output_size),dtype=torch.float32).cuda()
            index = torch.arange(max_time_step).cuda()
            mask[index,A] = torch.tensor(1,dtype=torch.float32).cuda()
