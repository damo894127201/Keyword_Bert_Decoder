...
if __name__ == '__main__':
    ...
    for epoch in range(startEpoch,config.num_Epochs):
        for iter,(batch_x,batch_y,batch_source_lens,batch_target_lens,batch_keyword,batch_keyword_lens) in enumerate(loader):
            ...
            batch_keyword = batch_keyword.cuda()
            batch_keyword_lens = torch.as_tensor(batch_keyword_lens).cuda()
            # outputs: [max_time_step,batch_size,vocab_size]
            outputs,GlobalCoverageLoss = model(batch_x,batch_source_lens,batch_y_cuda,batch_keyword,batch_keyword_lens,config.TeacherForcingRate)
            # 处理PAD填充位，避免对其进行损失计算
            ...
            GlobalCoverageLoss = GlobalCoverageLoss.cpu()
            ...
            if step % print_every == 0:
                ...
                for eval_iter, (batch_eval_x,batch_eval_y,batch_eval_source_lens,batch_eval_target_lens,batch_eval_keyword,batch_eval_keyword_lens) in enumerate(eval_loader):
                    ...
                    batch_eval_keyword = batch_eval_keyword.cuda()
                    batch_eval_keyword_lens = torch.as_tensor(batch_eval_keyword_lens).cuda()
