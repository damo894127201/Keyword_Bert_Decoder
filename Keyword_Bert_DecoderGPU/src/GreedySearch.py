...
def GreedySearch(decoderResult,output_size,bertModel,keywords,keywordsEmbedding,flag=True):
    ...
    # 定义三个tensor，用以指示不同模块
    type0 = torch.tensor(0).cuda()  # TransformerDecoder生成模块
    type1 = torch.tensor(1).cuda()  # Copy Attention Distribution 模块
    type2 = torch.tensor(2).cuda()  # Keyword Attention Distribution 模块
    PAD_ID = torch.tensor(0).cuda() # [PAD]的ID

    if flag == True:
        # 训练阶段
        batch_word_prob_Distribution = torch.zeros((batch_size,output_size),dtype=torch.float32).cuda() # 存储当前预测结果的概率分布
        # 当前解码结果对应的Embedding
        batch_word_Embedding = torch.zeros((batch_size, 1, embedding_size),dtype=torch.float32).cuda()
        # 遍历每一条数据，获取其解码结果
        for i in range(batch_size):
            prob_list = torch.tensor([transformer_value[i][0], Proba_copy_value[i][0], Proba_keyword_value[i][0]]).cuda()
            ...
            if typeIndex[0] == type0:
                ...
                # 遍历word中每个字符，去除PAD位
                word = torch.tensor([id for id in word if id != PAD_ID]).cuda()
        ...
        batch_word_ID = torch.zeros((batch_size, max_word_len)).cuda()  # 每条预测结果中都存在大量的[PAD]符号(ID为0),在输出预测结果时需要去除
        # 当前解码结果对应的Embedding
        batch_word_Embedding = torch.zeros((batch_size, 1, embedding_size),dtype=torch.float32).cuda()
        # 预测阶段
        # 遍历每一条数据，获取其解码结果
